...
ben_muerte_act <- 5000000 * sum(sapply(0:(64 - edad), function(i) {
  ...
  ben_anualidades <- v^(65-edad)*calcular_kpx(edad, 65-edad, tabla_mort)*13*300000 * sum(sapply((0:50), function(m) v^(m+1)* calcular_kpx(65, m, tabla_mort)))
...
```{r}
primas.hombres <- sapply((1:45), function(p) calcular_primas(p, qx.hombres))
primas.mujeres <- sapply((1:45), function(l) calcular_primas(l, qx.mujeres))
```
...
## Funciones de apoyo para la prima nivelada

### Función de sobrevivencia
```{r Función kpx}
kpx <- function(x, k, y, qx, w){
  
  #' @param x edad cumplida
  #' @param k años por sobrevivir
  #' @param qx dataframe con las probabilidades de muerte
  #' @param w edad final de la tabla
  #' @return probabilidad de sobrevivir k años de un inidividuo de edad x
  
  if (x %% 1 != 0 || k %% 1 != 0 || x < 0 || k < 0) {
    #stop("Deben de ser enteros positivos.")
  }
  
  if (x > w){
    stop("La edad en años cumplidos debe ser menor que la edad final de la tabla")
  }
  
  Sx <- 1
  anno <- which(colnames(qx) == y)
  
  if (k >= 1){
    for (i in 0:(k-1)){
      if (x + i <= w){
        Sx <- Sx*(1 - qx[[x+i+1, anno]])
        anno <- anno+1
      }
    }
  }
...
### Seguro completo anual: Ax
```{r}
Ax <- function(x, qx, y, w, i){
  ...
  #' @param w edad final de la tabla
  #' @return valor de un seguro completo anual para una persona de edad x
  ...
  for (j in 0:(w-x-1)){
      ...
      sobrevivencia <- kpx(x, j, y, qx, w)
      muerte <- qx[[x+1+j, anno+j]]
      descuento <- (1+i)**(-(j+1))
      Ax <- Ax + (sobrevivencia*muerte*descuento)
...
### Seguro temporal anual: Axn
```{r}
Axn <- function(x, n, y, qx, w, i){
  
  #' @param x edad cumplida
  #' @param n años de duracion por los cuales cubre el seguro
  #' @param y año actual
  #' @param qx dataframe con las probabilidades de muerte
  #' @param w edad final de la tabla
  #' @param i tasa de interes
  #' @return valor del seguro temporal anual para una persona de edad x, que cubre si muere antes de n años
  
  if ((x+n)>w){
    stop("El individuo de edad"+ x + "no puede comprar un seguro de" + n + "annos")
  }
  ...
  for (j in 0:(n-1)){
    ...
    muerte <- qx[[x+1+j, anno+j]]
    descuento <- (1+i)**(-(j+1))
    Axn <- Axn + (sobrevivencia*muerte*descuento)
  }
...
### Funcion para calcular prima nivelada

```{r}
prima.nivelada <- function(base.empleados, y) {
    df <- data.frame(vp = numeric(0), anualidad = numeric(0))
  
  j <- ((1.04 / 1.03) - 1)
  v <- 1/(1+j)
  for (i in 1:nrow(empleados)) {
    x <- base.empleados[["edad"]][i]
    
    if (base.empleados[["sexo"]][i] == "M") {
      prob <- qx.hombres
    } else {
      prob <- qx.mujeres
    }
    suma.1 <- 5000000 * Axn(x, 65 - x, y, prob, 115, j)
    #beneficio en caso de muerte del pensionado
    suma.2 <- 1000000 * Ax(x, prob, y, 115, j)
    #Beneficio de pension y aguinaldo por vejez
    suma.3 <- 300000 * 13 * Ax(x, prob, y, 115, j)
    
    vp.salidas <- suma.1 + suma.2 + suma.3
    #anualidad
    vp.entradas <- sum(sapply(1:(65 - x), function(r) v**r * calcular_kpx(x, r, prob)))
    
    df <- rbind(df, data.frame(vp = vp.salidas,  anualidad = vp.entradas))
  }
  return(df)
}
```
```{r}
prima.nivelada.resultado <- prima.nivelada(empleados, 2024)
```

```{r}
prima_nivelada <- sum(prima.nivelada.resultado$vp) / sum(prima.nivelada.resultado$anualidad)
```

###Metodos de reducciones de primas en un 10%
# En el primer mteodo se va a reducir el monto de la pension por aproximacion manual
# Al reducri los pagos mensuales los costos mensuales se disminuyen, para la empresa seria beneficioso
# ya que necesitaria menos fondos para cubrir sus gastos, sin embargo trae efectos negativos en los empleados
# sobretodo a futuro ya que el monto de la pension se va a reducir
```{r}
...
# Segundo metodo consiste en que ya las personas no van a poder jubilarse hasta la edad de 66
# Esto genera que se tengan que hacer menos pagos de pensiones, y a su vez que los empleados
# van a aportar por mas annos al fondo de pensiones, de igual forma el valor presete disminuye
# Y para los empleados no todo es negatico en parte,los empleados podran acumular beneficios por mas tiempo.
...
Inicialmente, para determinar la distribución a utilizar en la generación de números aleatorios, se crea un gráfico para observar la función de sobrevivencia de las probabilidades de muerte de la SUPEN.
...
# Gráfico
ggplot(data = data.frame(edad = c(21:115), datos = densidad.h)) +
  geom_density(aes(x = datos)) +
  theme_minimal()

ggplot(data = data.frame(edad = c(21:115), datos = densidad.m)) +
  geom_density(aes(x = datos)) +
  theme_minimal()
```
...
prob.muerteh <- sapply(1:45, function(j) sapply(((21 + j)):116, function(k) qx.hombres[[k, 25 + k]]))
...
prob.muertem <- sapply(1:45, function(j) sapply(((21 + j)):116, function(k) qx.mujeres[[k, 25 + k]]))
...
# Anualidades
anh <- sapply(1:filmat, function(n) sum((1 + j) ^ -(0:idh[n])))
anm <- sapply(1:filmat, function(n) sum((1 + j) ^ -(0:idm[n])))
