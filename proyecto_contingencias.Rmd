...
Posteriormente, los gráficos de comparación de primas. Primero se compararán los percentiles 50 y 90 de las primas estocásticas obtenidas, para luego hacer la comparación entre las primas estocásticas y las primas deterministas. Para esto, se calculará el promedio de los cuartíles 50 y 90 de las primas estocásticas y el promedio de las primas determinísticas.
...
promedio.primas <- data.frame(
  primas = names(df.primash),
  promedios = colMeans(df.primash / 1000000)
)

# Nombres para las categorías del eje x
primas = c("Cuartíl 50", "Cuartíl 90", "Determinísticas")

# Comparación entre las primas originales con los percentiles 50 y 90 de las primas estocásticas (hombres)
ggplot(promedio.primas, aes(x = primas, y = promedios, fill = primas)) +
  geom_bar(stat = "identity") +
  theme_minimal() +
  labs(x = "Primas",
       y = "Promedio (en millones)",
       caption = "Fuente: Elaboración propia") +
  scale_fill_manual(values = c("cyan2", "violet", "salmon")) +
  theme(legend.position = "none") +
  scale_x_discrete(labels = primas)

# De igual manera se hace el gráfico para las mujeres
promedio.primasm <- data.frame(
  primas = names(df.primasm),
  promedios = colMeans(df.primasm / 1000000)
)

ggplot(promedio.primasm, aes(x = primas, y = promedios, fill = primas)) +
  geom_bar(stat = "identity") +
  theme_minimal() +
  labs(x = "Primas",
       y = "Promedio (en millones)",
       caption = "Fuente: Elaboración propia") +
  scale_fill_manual(values = c("aquamarine2", "blueviolet", "coral")) +
  theme(legend.position = "none") +
  scale_x_discrete(labels = primas)

rm(promedio.primas, primas)
...
ggplot(primas.combinadas, aes(x = edad)) +
  ...
  scale_color_manual(
    name = "Género",
    values = c("Hombres" = "blue", "Mujeres" = "pink")) +
  geom_hline(yintercept = (prima_nivelada / 1000000), linetype = "dashed", linewidth = 1, color = "magenta3") +
  theme_minimal()
