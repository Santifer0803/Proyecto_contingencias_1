...
```{r}
primas.hombres_metodo1 <- sapply((21:65), function(p) calcular.primas.metodo1(p, qx.hombres))
primas.mujeres_metodo1 <- sapply((21:65), function(l) calcular.primas.metodo1(l, qx.mujeres))
```
...
```{r}
datos.hombres[["porcentaje.metodo1"]] <- datos.hombres[["primas_metodo1"]] / datos.hombres$primas
datos.mujeres[["porcentaje.metodo1"]] <- datos.mujeres[["primas_metodo1"]] / datos.mujeres$primas
```
...
```{r}
# Hacemos un sapply para poder obtener la prima de cada edad, tanto de homres como de mujers
primas.hombres_metodo2 <- sapply((21:67), function(p) calcular.primas.metodo2(p, qx.hombres))
primas.mujeres_metodo2 <- sapply((21:67), function(l) calcular.primas.metodo2(l, qx.mujeres))
```

```{r}
# Creamos un nuevo df para poder guardar las primas que se van a pagar ahora hasta los 67 años.
primas.metodos <- data.frame("edad" = 21:67)

# Guardamos las primas en una columa nueva.
primas.metodos[["primas_metodo2.hombres"]] <- primas.hombres_metodo2
primas.metodos[["primas_metodo2.mujeres"]] <- primas.mujeres_metodo2
```

Ahora vamos a ver que, si sacamos el promedio a estas 67 primas para ambos sexos, y sacamos el promedio de las primeras 65 primas originales, de igual forma para ambos sexos, vamos a tomar el primer resultado y dividirlo entre el original de las 65 primas, se puede notar que se logra reducir en promedio las primas en un $10\%$.

```{r}
promedio.hombres <- mean(primas.metodos[["primas_metodo2.hombres"]]) / mean(datos.hombres[["primas"]])
promedio.mujeres <- mean(primas.metodos[["primas_metodo2.mujeres"]]) / mean(datos.mujeres[["primas"]])
```
...
idh <- unname(sapply(1:filmat, function(m) which(!matriz.rndh[m, ])[1]) - 1)
idm <- unname(sapply(1:filmat, function(m) which(!matriz.rndm[m, ])[1]) - 1)
...
modelo.estocastico <- function(matriz.prob, prom.primas, densidad, lista.primas){
  ...
  # Índice del primer falso
  id <- unname(sapply(1:filmat, function(m) which(!matriz.rnd[m, ])[1]) - 1)
...
df.primash <- data.frame(primas_originales = datos.hombres$primas,
...
df.primasm <- data.frame(primas_originales = datos.mujeres$primas,
...
En esta sección se pondrán todos los gráficos que se presenten para el proyecto.

```{r comparacion_empleados activos}
...
ggplot(df.primash, aes(x = c(21:65))) +
  geom_line(aes(y = primas_originales, color = "Primas determinísticas")) +
  geom_line(aes(y = primas_cuartil_50, color = "Cuartíl 50 de Primas estocásticas")) +
  geom_line(aes(y = primas_cuartil_90, color = "Cuartíl 90 de Primas estocásticas")) +
  labs(
    x = "Edades",
    y = "Prima",
    caption = "Fuente: elaboración propia"
  ...
  scale_colour_manual(
    name = "Primas",
    values = c(
      "Primas determinísticas" = "skyblue",
      "Cuartíl 50 de Primas estocásticas" = "indianred",
      "Cuartíl 90 de Primas estocásticas" = "purple"
    )
  ) +
  theme_minimal()
...
```{r proyeccionfinac_muertes_empleados}
...
ggplot(costos.combinados, aes(x = anno, y = costo, color = genero)) +
  geom_line(size = 1) +
  labs(title = "",
       x = "Año",
       y = "Costo ajustado por inflación") +
  scale_color_manual(values = c("Hombres" = "blue", "Mujeres" = "pink")) +
  theme_minimal()
```

```{r comparacion_primas_hombres, warning=FALSE}
# Extraemos las columnas que ocupamos de cada df
primas.original.h <- datos.hombres[, c("edad", "primas")]
primas.metodo1.h <- datos.hombres[, c("edad", "primas_metodo1")]
primas.metodo2.h <- primas.metodos[, c("edad", "primas_metodo2.hombres")]

# Fusionamos los df en uno solo por la edad
primas.combinadas.h <- merge(primas.original.h, primas.metodo1.h, by = "edad", all = TRUE)
primas.combinadas.h <- merge(primas.combinadas.h, primas.metodo2.h, by = "edad", all = TRUE)


# Crear el gráfico combinado
ggplot(primas.combinadas.h) +
  geom_line(aes(x = edad, y = primas, color = "Original"), size = 0) +
  geom_line(aes(x = edad, y = primas_metodo1, color = "Método 1"), size = 0) +
  geom_line(aes(x = edad, y = primas_metodo2.hombres, color = "Método 2"), size = 0) +
  labs(title = "",
       x = "Edad",
       y = "Primas",
       color = "Tipo de Primas") +
  scale_color_manual(values = c("Original" = "black", "Método 1" = "blue", "Método 2" = "red")) +
  theme_minimal()

```

```{r comparacion_primas_mujeres, warning=FALSE}
# Extraemos las columnas que ocupamos de cada df
primas_original <- datos.mujeres[, c("edad", "primas")]
primas_metodo1 <- datos.mujeres[, c("edad", "primas_metodo1")]
primas_metodo2 <- primas.metodos[, c("edad", "primas_metodo2.mujeres")]

# Fusionamos los df en uno solo por la edad
primas_combinadas <- merge(primas_original, primas_metodo1, by = "edad", all = TRUE)
primas_combinadas <- merge(primas_combinadas, primas_metodo2, by = "edad", all = TRUE)


# Crear el gráfico combinado
ggplot(primas_combinadas) +
  geom_line(aes(x = edad, y = primas, color = "Original"), size = 0) +
  geom_line(aes(x = edad, y = primas_metodo1, color = "Método 1"), size = 0) +
  geom_line(aes(x = edad, y = primas_metodo2.mujeres, color = "Método 2"), size = 0) +
  labs(title = "",
       x = "Edad",
       y = "Primas",
       color = "Tipo de Primas") +
