...
fila.muerte <- function(fila){
  ...
  fila[(which(fila == FALSE)[1] + 1):length(fila)] <- FALSE
  return(fila)
}
```
...
matriz.rndh <- matrix(data = pnorm(rnorm(ncol(matriz.probh) * nrow(matriz.probh), mean(densidad.h), sd = sd(densidad.h)), mean = mean(densidad.h), sd = sd(densidad.h)), nrow = nrow(matriz.probh), ncol = ncol(matriz.probh))

matriz.rndm <- matrix(data = pnorm(rnorm(ncol(matriz.probm) * nrow(matriz.probm), mean(densidad.m), sd = sd(densidad.m)), mean = mean(densidad.m), sd = sd(densidad.m)), nrow = nrow(matriz.probm), ncol = ncol(matriz.probm))
...
idh <- sapply(1:nrow(matriz.rndh), function(m) which(matriz.rndh[m, ] == FALSE)[1]) - 1
idm <- sapply(1:nrow(matriz.rndm), function(m) which(matriz.rndm[m, ] == FALSE)[1]) - 1

# Anualidades
anh <- sapply(1:length(idh), function(n) sum((1 + j) ^ -(0:idh[n])))
anm <- sapply(1:length(idm), function(n) sum((1 + j) ^ -(0:idm[n])))
