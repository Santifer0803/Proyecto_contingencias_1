...
Se crea una función que extrae las probabilidades de muerte de más de un año, para un individuo.

```{r fun_kpx}
#' Calcula las probabilidades de sobrevivencia de un individuo.
#'
#' Esta función calcula las probabilidades de sobrevivencia de un individuo de 
#' de edad \code{x}, con base a la cantidad de años \code{k}, iniciando en el
#' año 2024, con una \code{tabla} de mortalidad.
#'
...
#'
#' @return (Numeric) Vector de probabilidades de sobrevivencia.
#'
...
fila.muerte <- function(fila){
  #' Toma el índice de un vector, una posición antes de un falso.
  #'
...
Se procede a realizar la funcion que calcula la proyección de los beneficios de los pesionados que mueren.
...
def.proyeccion.finac.pen <- function(df.pensionados.fallecidos) {
  ...
  # El for recorre cada año y cada cantidad de muertos por anno, considera la inflacion
...
Para el segundo método de reducción en un 10% de las primas, se tomaron varios supuestos, primero, ahora las personas se pueden pensionar hasta la edad de 67 años y no a los 65, segundo, el monto de la pensión se redujo en 2 mil colones, lo que puede parecer un monto bajo sin embargo al ser un monto que se paga anualmente hasta que la persona muera se puede convertir en suma bastante considerable, además se redujo el beneficio de muerte de empleado activo a dos millones novecientos mil, y de igual forma el monto de beneficio en caso de muerte del pensionado se disminuyó a setecientos sesenta mil.
...
modelo.estocastico <- function(matriz.prob, prom.primas, densidad, lista.primas){
  ...
  matriz.rnd <- matrix(data = pnorm(rnorm(colmat * filmat, 0, sd = 1), mean = 0, sd = 1), nrow = filmat, ncol = colmat)
